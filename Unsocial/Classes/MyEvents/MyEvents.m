...
#import "Person.h"
#import "SearchEventsFromCodeKeyword.h"

BOOL allInterest2 = NO;
// not in use added by pradeep on 11 june 2011
//int currentindexforscrolltableview2;
NSString *localuserid, *localdistance2;
int whichsegmentselectedlasttime4event2;
int arivefirsttime4event2;


// added by pradeep on 15 feb 2011
BOOL isfilterclick2 = NO;
...
BOOL vieweventdetail2 = NO;


// added by pradeep on 16 dec 2010 for calendar
int whichsegmentselectedlasttime4cal2;
int arivefirsttime4cal2;
...
- (void)createControls {
	...
	else
	{
		...
		mySearchBar = [[[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, 320, 35)] autorelease]; //(0.0, 43.0, 320, 35)
		...
		mySearchBar.placeholder = @"type code or keyword";
		
		// note: here you can also change its "tintColor" property to a different UIColor
		
		[self.view addSubview:mySearchBar];
		
		NSArray *segmentTextContent = [NSArray arrayWithObjects:@"PAST", @"UPCOMING", nil];
		segmentedControl = [[UISegmentedControl alloc] initWithItems:segmentTextContent];
		segmentedControl.frame = CGRectMake(20.0, 38.0, 280, 35);//CGRectMake(40, 378, 240, 35);  (20, 5, 280, 35)
		...
		segmentedControl.selectedSegmentIndex = whichsegmentselectedlasttime4event2;
...
/*- (void)rightbtn_OnClick {
		 ...
		 //WhereIam = 0;
		 arivefirsttime4event2 = 0;
		 // added by pradeep on 27 jan 2011 for fixing error for scrolling table for current date based on value of currentindexforscrolltableview2 variable, i.e. first time it works fine but when go back and change the tab and come again to list view, currentindexforscrolltableview2 contains last assigned value and give out of bound array during scrolling
		 currentindexforscrolltableview2 = 0;
...
- (void)leftbtn_OnClick {
	...
	arivefirsttime4event2 = 0;
	arivefirsttime4cal2 = 0;
	whichsegmentselectedlasttime4cal2 = 0;
	whichsegmentselectedlasttime4event2 = 0;
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
	...
	else */
	{
		...
		SearchEventsFromCodeKeyword  *se = [[SearchEventsFromCodeKeyword alloc]init];
...
- (void)startProcess4ChangedSegment
{
	...
	//int temp4arivefirsttime4event2 = 0;
	...
	if (!itemTableView)
	{
		if (whichsegmentselectedlasttime4event2 == segmentedControl.selectedSegmentIndex)
			arivefirsttime4event2 = 0;
		//arivefirsttime4event2 = 0;
	}
	
	//int temp4whichsegmentselectedlasttime4event = whichsegmentselectedlasttime4event2;
	
	if(arivefirsttime4event2==0)
	{
		arivefirsttime4event2 = 1;
		whichsegmentselectedlasttime4event2 = segmentedControl.selectedSegmentIndex;
		if (!allInterest2) // i.e. allInterest2=NO i.e. All tab is selected
		{
			//NSLog(@"for all events");
			if (selectedSegment==0)
			{
				[self getEventData:@"mypastevents4codekey" :@"all"];
				NSLog(@"0");
			}
			else if (selectedSegment==1)
			{
				NSLog(@"1");
				//self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(rightbtn_OnClick)] autorelease];
				[self getEventData:@"myupcomingevents4codekey" :@"future"];
			}
			
		}
		[self getdifferentdatesforsectionwhensegmentchange:stories];
		
	} // close arivefirsttime4event2 if
	else 
	{
		if (whichsegmentselectedlasttime4event2 != segmentedControl.selectedSegmentIndex)
		{
	...
	{				
		...
		if([stories count] > 0)
		{
			itemTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 75, 320, 340) style:UITableViewStyleGrouped];
			itemTableView.delegate = self;
			itemTableView.dataSource = self;
			itemTableView.rowHeight = 55;
			itemTableView.autoresizesSubviews = YES;
			...
			//NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0]; // currentindexforscrolltableview2
	...
	if ([stories count] == 0)
	{
		...
		if (selectedSegment == 1)
		{
			...
			recnotfound.text = @"There are currently no premium events. Try again later.";
		}
		else if (selectedSegment == 2)
		{
			recnotfound.text = @"There are currently no premium events. Try again later.";
...
/*- (void) getindexforscrollingtableview
{
	...
	if ([filteredArray count] > 0)
	{
		for (int i = 0; i < [stories4cal count]; i++) 
		{
			...
			if ([[stories4cal objectAtIndex:i] isEqualToString:strdtdeep])
			{
				NSLog([NSString stringWithFormat:@"%d", i]);
				//currentindexforscrolltableview2 = i;		
				break;
			}
		}
	}
	//if (currentindexforscrolltableview2==0)
	else
	{
		for (int i = 0; i < [stories4cal count]; i++) 
		{
			NSDate *futuredt = [[NSDate alloc] init];
			futuredt = [dateFormatter1 dateFromString:[stories4cal objectAtIndex:i]];
...
- (void) getdifferentdatesforsectionwhensegmentchange: (NSMutableArray *) tempstories4cal // since we have to change the calendar *(i.e. item exist for changed segment i.e. saved or around me) each item
{
	...
	// not in use below commented section added by pradeep on 24 june 2011
	/*NSDate *today = [NSDate date];
	NSDateFormatter *dateFormatter1 = [[NSDateFormatter alloc] init]; 
	[dateFormatter1 setDateFormat:@"M/d/yyyy"];
	NSString *strdtdeep = [dateFormatter1 stringFromDate:today];
	*/
	...
	for (int i = 0; i < [tempstories4cal count]; i++) 
	{
		...
		if ([tempnsmutableary count] > 0)
		{
			for (int j = 0; j < [tempnsmutableary count]; j++)
			{
				...
				// commented by pradeep on 24 june 2011 for grouping month n year wise
				if ([[tempnsmutableary objectAtIndex:j] isEqualToString:[[tempstories4cal objectAtIndex:i] objectForKey:@"eventdate"]])
				{
					isalreadyexist = YES;
					break;
				}
				// end comment on 24 june 2011
				
				// added by pradeep on 24 june 2011 for grouping table month n year wise
				// added by pradeep on 24 june 2011 for implementing items group by on the basis of month n year
				
				/*NSDate *evedate;
				
				NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
				[dateFormat setDateFormat:@"M/d/yyyy"];
				NSString *strdtnew = [[tempstories4cal objectAtIndex:i] objectForKey:@"eventdate"];
				NSLog(@"date: %@", strdtnew);
				evedate = [dateFormat dateFromString:strdtnew];
				//NSLog(@"month: %B", evedate);
				//NSLog(@"year: %Y", evedate);
				//NSString *dateString = [dateFormat stringFromDate:evedate];
				...
				[dateFormat2 setDateFormat:@"EEE MMMM d, yyyy"];
				NSString *dateString = [dateFormat2 stringFromDate:evedate];
				NSLog(@"complete date with name: %@", dateString);
				...
				NSLog(@"New format of event date: %@ %@", strmonth, stryear);
				
				[myArray release];
				[myArray2 release];
								
				if ([[tempnsmutableary objectAtIndex:j] isEqualToString:[NSString stringWithFormat:@"%@ %@", strmonth, stryear]])
				{
					isalreadyexist = YES;
					break;
				}*/
				
				// end 24 june 2011 by pradeep
				// end 24 june 2011
			}
			...
			if (!isalreadyexist)
			{
				...
				[tempnsmutableary addObject:[NSString stringWithFormat:@"%@ %@", strmonth, stryear]];*/
				
				// end 24 june 2011 by pradeep
				
				// for getting index to scroll tableview automatically first time
				//if ([[tempnsmutableary objectAtIndex:([tempnsmutableary count]-1)] isEqualToString:strdtdeep])
				//	currentindexforscrolltableview2 = ([tempnsmutableary count]-1);
			}
	...
	[stories4grp removeAllObjects];
	//eventtoggledateary = [NSMutableArray arrayWithArray:tempnsmutableary];
	[stories4grp addObjectsFromArray:tempnsmutableary];
...
/*- (BOOL) getDataFromFile4EventDistance {
	...
	if([fileManager fileExistsAtPath:path]) {
		...
		localdistance2 = [[tempArray objectAtIndex:0] setMilesForEvents];
...
- (void) getEventData:(NSString *) eventflg: (NSString *) eventsubflg {
	...
	NSLog(@"Sending....");
	NSString *urlString = [NSString stringWithFormat:@"/iphone/iPhoneReqPage1_1.aspx?flag=%@&latitude=%@&longitude=%@&userid=%@&datetime=%@&distance=%@&eventflag=%@&eventsubflag=%@",@"getevents",gbllatitude,gbllongitude,gbluserid,usertime,localdistance2,eventflg,eventsubflg];
...
- (void)parseXMLFileAtURL:(NSString *)URL {
	...
	stories4grp = [[NSMutableArray alloc] init];
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
	...
	{
		// commented by pradeep on 24 june 2011
		...
		[dateFormatt setDateFormat:@"M/d/yyyy"];
		NSString *strdtnew2 = string;
		NSLog(@"date deep: %@", strdtnew2);
		evedate2 = [dateFormatt dateFromString:strdtnew2];
		//NSLog(@"month: %B", evedate);
		//NSLog(@"year: %Y", evedate);
		//NSString *dateString = [dateFormat stringFromDate:evedate];
		//NSLog(@"complete date with name: %@", dateString);
		
		NSMutableArray *myArray11 = [[NSMutableArray alloc] init];
		NSMutableArray *myArray22 = [[NSMutableArray alloc] init];
		
		[dateFormatt release];
		
		NSDateFormatter *dateFormat22 = [[NSDateFormatter alloc] init];
		[dateFormat22 setDateFormat:@"EEE MMMM d, yyyy"];
		NSString *dateString2 = [dateFormat22 stringFromDate:evedate2];
		NSLog(@"complete date with name deep: %@", dateString2);
		
		[dateFormat22 release];
		
		
		[myArray22 setArray:[dateString2 componentsSeparatedByString:@","]];
		
		NSString *strdaymondt2 = [myArray22 objectAtIndex:0];
		NSString *stryear2 = [myArray22 objectAtIndex:1];
		
		[myArray11 setArray:[strdaymondt2 componentsSeparatedByString:@" "]];		
		
		//NSString *strday = [myArray objectAtIndex:0];
		NSString *strmonth2 = [myArray11 objectAtIndex:1];
		//NSString *strdate = [myArray objectAtIndex:2];
		//NSString *stryr = stryear;
		
		NSLog(@"New format of event date deep: %@ %@", strmonth2, stryear2);
		
		// added by pradeep on 7 june 2011 due to memory issue
		[myArray11 release];
		[myArray22 release];
		
		[eventdate appendString:[NSString stringWithFormat:@"%@ %@", strmonth2,stryear2]];
	...
	{
		// commented by pradeep on 26 june 2011
		//[eventdateto appendString:string]; 
		// 26 june 2011 now we will use eventdateto for eventdate (from) since list is grouped by month n year so i used eventdate (From) for that purpose, since we have to show event date also on the list so i am using eventdateto for eventdate (from)
		
		// added by pradeep on 26 june 2011 for implementing items group by on the basis of month n year
		
		NSDate *evedate3;
		
		NSDateFormatter *dateFormatt3 = [[NSDateFormatter alloc] init];
		[dateFormatt3 setDateFormat:@"M/d/yyyy"];
		NSString *strdtnew3 = string;
		NSLog(@"date deep: %@", strdtnew3);
		evedate3 = [dateFormatt3 dateFromString:strdtnew3];
		//NSLog(@"month: %B", evedate);
		//NSLog(@"year: %Y", evedate);
		//NSString *dateString = [dateFormat stringFromDate:evedate];
		//NSLog(@"complete date with name: %@", dateString);
		
		NSMutableArray *myArray111 = [[NSMutableArray alloc] init];
		NSMutableArray *myArray222 = [[NSMutableArray alloc] init];
		
		[dateFormatt3 release];
		
		NSDateFormatter *dateFormat33 = [[NSDateFormatter alloc] init];
		[dateFormat33 setDateFormat:@"EEE MMMM dd, yyyy"];
		NSString *dateString3 = [dateFormat33 stringFromDate:evedate3];
		NSLog(@"complete date with name deep: %@", dateString3);
		
		[dateFormat33 release];
		
		
		[myArray222 setArray:[dateString3 componentsSeparatedByString:@","]];
		
		NSString *strdaymondt3 = [myArray222 objectAtIndex:0];
		//NSString *stryear3 = [myArray222 objectAtIndex:1];
		
		[myArray111 setArray:[strdaymondt3 componentsSeparatedByString:@" "]];		
		
		//NSString *strday = [myArray objectAtIndex:0];
		NSString *strmonth3 = [myArray111 objectAtIndex:1];
		NSString *strdate3 = [myArray111 objectAtIndex:2];
		//NSString *stryr = stryear;
		
		
		
		NSLog(@"New format of event date deep: %@ %@", [[strmonth3 substringWithRange:NSMakeRange(0, 3)] uppercaseString], strdate3);
		
		// added by pradeep on 7 june 2011 due to memory issue
		[myArray111 release];
		[myArray222 release];
		
		[eventdateto appendString:[NSString stringWithFormat:@"%@ %@", [[strmonth3 substringWithRange:NSMakeRange(0, 3)] uppercaseString],strdate3]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(eventdate ==  %@)", [stories4grp objectAtIndex:indexPath.section]];
	...
	// commented by pradeep on 26 june 2011 since 'my event' section is for premium events
	...
	// commented by pradeep on 16 june 2011 for replacing imageview to label with orange bgrnd n month n date
	/*CGRect frame;
	frame.size.width = 40;
	...
	UILabel *lblDate = [UnsocialAppDelegate createLabelControl:[[temp4filterary2 objectAtIndex:indexPath.row] objectForKey:@"eventdateto"] frame:frame txtAlignment:UITextAlignmentCenter numberoflines:2 linebreakmode:UILineBreakModeWordWrap fontwithname:kAppFontName fontsize:15 txtcolor:[UIColor whiteColor] backgroundcolor:[UIColor orangeColor]];
	
	// Get the Layer of any view
	CALayer * l = [lblDate layer];
	[l setMasksToBounds:YES];
	[l setCornerRadius:10.0];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
	...
	headerLabel.text = [NSString stringWithFormat:@"%@", [stories4grp objectAtIndex:section]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(eventdate ==  %@)", [stories4grp objectAtIndex:section]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([[[temp4filterary2 objectAtIndex:indexPath.row] objectForKey:@"eventtype"] isEqualToString:@"sponsoredevent"])
	{
		...
		//whichsegmentselectedlasttime4cal2 = segmentedControlcal.selectedSegmentIndex;
