...
@implementation SearchEventsFromCodeKeyword
...
- (void)viewWillAppear:(BOOL)animated {
	...
	WhereIam = 0;
	NSLog(@"SearchEventsFromCodeKeyword view will appear");
...
- (void)segmentAction:(id)sender {
	...
	if(arivefirsttime4sponevent==0)
	{
		...
		//if (!allInterest) // i.e. allInterest=NO i.e. All tab is selected
		{
			...
			{
				[self getEventData:@"searcheventsfromcodekeyword" :searchkey]; // getting searchkey from searchviewcontrollerimproved.m when user select event from segment controller
				NSLog(@"0");
			}
