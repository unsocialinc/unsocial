...
- (void)viewWillAppear:(BOOL)animated {
	...
	UILabel *heading = [UnsocialAppDelegate createLabelControl:@"People" frame:CGRectMake(15, 0, 290, 43) txtAlignment:UITextAlignmentLeft numberoflines:1 linebreakmode:UILineBreakModeWordWrap fontwithname:kAppFontName fontsize:kAppMainHeadingFontSize txtcolor:[UIColor orangeColor] backgroundcolor:[UIColor clearColor]];
...
// added by pradeep on 1 june 2011 for Shake Gesture feature
...
- (void)getUserList {
	...
	NSLog(@"Sending....");
	NSString *urlString = [NSString stringWithFormat:@"/iphone/iPhoneReqPage1_1.aspx?flag=%@&latitude=%@&longitude=%@&userid=%@&datetime=%@&distance=%@&flag2=%@&eventid=%@",@"getpeople",gbllatitude,gbllongitude,gbluserid,usertime,@"0",@"sponeventpeople",[liveNowEventID objectAtIndex:0]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSLog(@"Image url for attendee users: %@", [imageURLs4Lazy objectAtIndex:indexPath.row]);
	...
	if ([[[stories objectAtIndex:indexPath.row] objectForKey:@"username"] length] > 18)			
	...
	NSArray *aryMiles = [[stories objectAtIndex:indexPath.row] objectForKey:@"currentdistance"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	peoplesUserProfile.camefromwhichoption = @"liveattendeelist";
