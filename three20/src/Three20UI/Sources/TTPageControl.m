...
- (void)drawRect:(CGRect)rect {
  if (_numberOfPages > 1 || _hidesForSinglePage) {
    ...
    CGFloat totalWidth = (dotWidth * _numberOfPages) - (boxStyle.margin.left + boxStyle.margin.right);
	  // commented by pradeep on 14 Jan 2011 for changing position of page control's dot on spring board
	  // ************* 14 jan 2011 start ***********
    CGRect contentRect = CGRectMake(round(self.width/2 - totalWidth/2),
	  ...
	  // added by pradeep on 14 Jan 2011 for changing position of page control's dot on spring board
	  //CGRect contentRect = CGRectMake(round(self.width/2 - totalWidth/2), 0, dotSize.width, dotSize.height);
